...
import type { QuizQuestionBoolSingleFormSchema } from "../createQuestionFormSchemas"

export function BoolSingleForm() {
  const { control, register, formState: { errors }, watch, trigger } = useFormContext<QuizQuestionBoolSingleFormSchema>()
  ...
  const updateChecked = (index: number, value: boolean) => {
    ...
    if(value) {
      fields.forEach((_, i) => {
        update(i, { ...choices[i], correct: i === index })
      })
    } else {
    ...
    <Field>
      ...
      <FieldContent>
        ...
        <FieldDescription className="mt-2">
          Añade al menos 2 opciones. Marca la respuesta correcta.
        </FieldDescription>
