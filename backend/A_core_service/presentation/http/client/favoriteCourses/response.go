...
type FavoriteCourseResponse struct {
	ID                  string   `json:"id"`
...
func (self *GetFavoriteCoursesRequest) getResponse(courses []typesenseentity.CourseDocument) []FavoriteCourseResponse {
	response := make([]FavoriteCourseResponse, len(courses))

	for i, course := range courses {
		response[i] = FavoriteCourseResponse{
			ID:                  course.ID,
			Slug:                course.Slug,
