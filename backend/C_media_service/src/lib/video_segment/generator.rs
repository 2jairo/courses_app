...
pub const NVENC_ENCODE_SESSIONS_LIMIT: usize = 8;
...
impl<'a> VideoGenerator<'a> {
    ...
    pub async fn process_resolutions(&self) -> anyhow::Result<()> {
        ...
        while r < resolutions.len() {
            let len = (r + NVENC_ENCODE_SESSIONS_LIMIT).min(resolutions.len());
...
// /// filesrc
// /// └─ decodebin3
// ///     ├─ videoscale → videorate → capsfilter → nvh265enc → h265parse → hlssink3:video
// ///     └─ audioconvert → audioresample → voaacenc → aacparse → hlssink3:audio
// /// ```
// fn parse_single_resolution_build_pipeline(
//     video_path: &str,
...
//         .field("height", res.h)
//         .field("framerate", &Fraction::new(res.target_framerate, 1))
//         // .field("format", if cpu { "I420" } else { "NV12" })
//         .build();
...
//     // hls
//     let playlist_location_buf = resolution_dir.join("index.m3u8");
//     let playlist_location = playlist_location_buf.to_str().unwrap();
//     let segment_location_buf = resolution_dir.join("%d.ts");
//     let segment_location = segment_location_buf.to_str().unwrap();

//     filesrc.set_property("location", video_path);
//     hlssink3.set_property("location", segment_location);
//     hlssink3.set_property("target-duration", CONFIG.segment_duration as u32);
//     hlssink3.set_property("playlist-length", 0u32);
//     hlssink3.set_property("max-files", 0u32);
//     hlssink3.set_property("playlist-location", playlist_location);
//     hlssink3.set_property_from_str("playlist-type", "vod");
//     pipeline.set_property("latency", 0u64); 
...
//         let audio_sink_pad = hlssink3.request_pad_simple("audio").unwrap();
//         let aacparse_src = aacparse.static_pad("src").unwrap();
//         aacparse_src.link(&audio_sink_pad)?;
//     }
...
// ///   filesrc
// ///   └─ decodebin3
// ///       ├─ tee ─┬─ queue ─ videoscale ─ videorate ─ capsfilter(1080p, 60fps) ─ nvh265enc ─ h265parse ─ hlssink3:video
// ///       │       ├─ queue ─ videoscale ─ videorate ─ capsfilter(720p, 60fps)  ─ nvh265enc ─ h265parse ─ hlssink3:video
// ///       │       
// ///       └─ tee ─┬─ queue ─ audioconvert ─ audioresample ─ voaacenc (v_bitrate1) ─ aacparse ─ hlssink3:audio
// ///               ├─ queue ─ audioconvert ─ audioresample ─ voaacenc (v_birtare2)─ aacparse ─ hlssink3:audio
// /// ```
// fn parse_multiple_resolutions_build_pipeline(
//     video_path: &str,
//     resolutions: &[(ResFrameBitrate, usize, PathBuf)],
// ) -> anyhow::Result<Pipeline> {
//     let pipeline = Pipeline::new();
...
//     pipeline.add_many(&[&filesrc, &decodebin3, &tee_audio, &tee_video])?;
//     filesrc.link(&decodebin3)?;
//     // audioconvert2.link(&audioresamlpe2)?;
//     // audioresamlpe2.link(&tee_audio)?;

//     for (i, (res, res_idx, resolution_dir)) in resolutions.iter().enumerate() {
//         let video_queue = ElementFactory::make("queue").build()?;
//         let videoscale = ElementFactory::make("videoscale").build()?;
//         let videorate = ElementFactory::make("videorate").build()?;
//         let capsfilter = ElementFactory::make("capsfilter").build()?;
...
//         hlssink3.set_property("playlist-location", playlist_location);
//         hlssink3.set_property_from_str("playlist-type", "vod");
//         // hlssink3.set_property("h", res.h); // custom prop (hlssink3_cp)
//         // hlssink3.set_property("framerate", res.target_framerate) // custop prop
