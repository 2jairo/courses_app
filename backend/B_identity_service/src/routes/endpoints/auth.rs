...
use crate::{config::CONFIG, error::{LocalErr, LocalErrKind, LocalResult}, extract::{Authenticated, Json}, models::entity::user, routes::dto::{auth::{LoginRequestBody, RefreshAccessTokenResponse, RegisterRequestBody, UserRequestsResponse}, common::Validated}, state::AppState};
...
pub async fn register(
    ...
    Json(Validated(body)): Json<Validated<RegisterRequestBody>>,
...
pub async fn login(
    State(AppState { users_service, jwt_service , .. }): State<AppState>,
    Json(Validated(body)): Json<Validated<LoginRequestBody>>,
) -> LocalResult<(CookieJar, Json<UserRequestsResponse>)> {
    let exists_cond = Condition::any()
